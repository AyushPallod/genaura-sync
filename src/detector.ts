import type { ScoredSkill, OverlapGroup, RatingReport } from './types.js';
...
interface OverlapMatch {
  type: 'trigger' | 'owns' | 'tag';
  value: string;
  skills: ScoredSkill[];
}

function findOverlaps(skills: ScoredSkill[]): OverlapMatch[] {
  const triggerMap = new Map<string, ScoredSkill[]>();
  const ownsMap = new Map<string, ScoredSkill[]>();
  const tagMap = new Map<string, ScoredSkill[]>();
  ...
  for (const skill of skills) {
    ...
    for (const own of toArray(skill.skillYaml?.owns)) {
      ...
      if (!ownsMap.has(normalized)) ownsMap.set(normalized, []);
...
function groupOverlaps(overlaps: OverlapMatch[]): OverlapGroup[] {
  // Group overlaps by the set of skills involved
  const groupMap = new Map<string, { domains: string[]; skills: ScoredSkill[] }>();
...
function generateRecommendationReason(best: ScoredSkill, alternatives: ScoredSkill[]): string {
  ...
  if (breakdown.identity >= 20) reasons.push('strong identity definition');
  if (breakdown.sharpEdges >= 20) reasons.push('comprehensive pitfall coverage');
  if (breakdown.validations >= 20) reasons.push('robust validations');
  if (breakdown.collaboration >= 20) reasons.push('clear collaboration rules');
...
function generateAlternativeUseCase(alt: ScoredSkill, best: ScoredSkill): string {
...
function findContradictions(skills: ScoredSkill[]): Contradiction[] {
...
export function analyzeOverlaps(skills: ScoredSkill[]): { overlaps: OverlapGroup[]; contradictions: Contradiction[] } {
