...
import type { ScoredSkill, RatingReport } from './types.js';
...
program
  ...
  .action(async (skillIds) => {
    ...
    try {
      ...
      const selected = scoredSkills.filter(s => 
        skillIds.some((id: string) => s.id.toLowerCase().includes(id.toLowerCase()))
      );
...
function colorScore(score: number): string {
  if (score >= 80) return brand.cyan.bold(score.toString());
  if (score >= 60) return brand.purple(score.toString());
...
function printTierSummary(skills: ScoredSkill[]) {
  const tiers = {
...
function printDetailedScores(skills: ScoredSkill[]) {
...
function printOverlaps(overlaps: any[]) {
  ...
  overlaps.slice(0, 5).forEach(overlap => {
    ...
    overlap.recommendation.alternatives.slice(0, 2).forEach((alt: any) => {
...
function printContradictions(contradictions: any[]) {
...
function printComparison(skills: ScoredSkill[]) {
  ...
  const keys = ['identity', 'sharpEdges', 'validations', 'collaboration'] as const;
