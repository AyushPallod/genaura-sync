...
export function getSourcePath() {
    ...
    if (existsSync(genauraSyncPath))
        return genauraSyncPath;
...
export function restoreBackup(backupPath, targetName) {
    const result = { success: true, restored: [], errors: [] };
...
export function backupTarget(target, backupDir) {
    if (!target.exists) {
        return null;
    }
    try {
...
export function syncSkillsToTarget(skills, target, options = {}) {
    const result = {
        target: target.name,
        linked: 0,
        skipped: 0,
...
export function getExistingSkillsInTarget(target) {
    if (!target.exists)
        return [];
    try {
 ...
 */
export function pullFromTarget(target, sourcePath, options = {}) {
